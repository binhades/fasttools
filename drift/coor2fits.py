...
def coor_central(obs_utc, dec=0, hr=0):

    lst = utc2lst(obs_utc)
    ra = lst - hr
    ra_arr = ra.deg

    dec_arr = np.array(ra_arr.shape)
    dec_arr[:] = dec
    print(dec_arr.shape)

    return np.array([ra_arr,dec_arr]) # shape: [2, leng]

def coor_offset(coor_b0, beam=1):
    if beam == 1:
        coor_bn = coor_b0
    else:
        coor_bn = coor_b0 # TODO

    return coor_bn

def radec2fits(hdul,beam=1,dec=0,hr=0):

    hdr = hdul[1].header
    tab = hdul[1].data
    obs_utc = hdul[1].data['DATE-OBS']

    coor_b0 = coor_central(obs_utc, dec=dec, hr=hr)
    coor_bn = coor_offset(coor_b0, beam=1)

    hdul[1].data['OBJ_RA']  = coor_bn[0,:]
    hdul[1].data['OBJ_DEC'] = coor_bn[1,:]

#    for i in range(len(obs_mjd)):
#        hdul[1].data['OBJ_RA']  = coor_bn[0,i]
#        hdul[1].data['OBJ_DEC'] = coor_bn[1,i]
